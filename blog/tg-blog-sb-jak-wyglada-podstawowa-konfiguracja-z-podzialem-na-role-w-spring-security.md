...
Poniżej znajduje się przykład podstawowej konfiguracji, w której:
- Dostęp do ścieżki `/admin/**` ma tylko rola `ADMIN`.
- Dostęp do ścieżki `/user/**` mają role `USER` oraz `ADMIN`.
- Dostęp do pozostałych ścieżek jest otwarty (publiczny).
