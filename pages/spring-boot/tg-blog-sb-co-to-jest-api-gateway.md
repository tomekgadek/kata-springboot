...
- **Routing:** Przekierowywanie żądań do odpowiednich mikroserwisów (często we współpracy z rejestrem usług, np. Eureka). – **Bezpieczeństwo:** Centralna weryfikacja autoryzacji (np. sprawdzanie tokenów JWT) przed wejściem do systemu. – **Rate Limiting i Filtrowanie:** Zabezpieczenie przed przeciążeniem oraz dodawanie niezbędnych nagłówków do zapytań (np. śledzenie ID).
